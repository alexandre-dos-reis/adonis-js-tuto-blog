  ...
  constructor() {
    ...
    document.addEventListener('turbo:before-render', (event) => {
      if (this.isPreviewRendered()) {
        ...
        event.detail.newBody.classList.remove(this.turboLoadingClass)
        // start fading out 1 frame later after opacity starts full
        requestAnimationFrame(() => {
          document.body.classList.add(this.turboLoadingClass)
        })
      } else {
        const isRestoration = event.detail.newBody.classList.contains(this.turboLoadingClass)
        if (isRestoration) {
        ...
        event.detail.newBody.classList.add(this.turboLoadingClass)
      }
    })
